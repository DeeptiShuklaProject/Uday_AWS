...
```mermaid
graph TD
    subgraph The Docker Architecture
        Internet["Internet (Port 80)"]
        
        subgraph Physical Linux Server (Host)
            Kernel["Linux Kernel"]
...
```mermaid
graph LR
    subgraph The Port Mapping Concept
        User["User on Internet"]
        
        subgraph Physical Server (IP: 203.0.113.50)
            HostPort["Host Port 80"]
            HostPort8080["Host Port 8080"]
            
            subgraph Container 1 (Web A)
                ContPort80A["Container Port 80"]
            end
            
            subgraph Container 2 (Web B)
