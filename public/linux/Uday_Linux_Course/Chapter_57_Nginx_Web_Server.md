...
```mermaid
graph TD
    subgraph Nginx_Reverse_Proxy_Architecture ["Nginx Reverse Proxy Architecture"]
        Internet["The Public Internet"]
        Firewall["Firewall (Port 80/443)"]
        
        subgraph The_DMZ ["The DMZ"]
            Nginx["Nginx Reverse Proxy<br/>(10.0.1.50)"]
            Static["Local SSD<br/>(Images, CSS, JS)"]
        end
        
        subgraph Internal_Network_Hidden ["Internal Network (Hidden)"]
...
```mermaid
graph LR
    subgraph The_PHP_FPM_Hand_off ["The PHP-FPM Hand-off"]
        User["User"]
        Nginx["Nginx (Port 80)"]
        HTML["index.html"]
