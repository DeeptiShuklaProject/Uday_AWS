...
- Explain what a process is and understand the PID (Process ID).
- List running processes using `ps` and its various flags.
- Monitor real-time system performance using `top` and `htop`.
- Send specific signals (like SIGTERM and SIGKILL) to processes using `kill`.
- Terminate multiple processes by name using `pkill` and `killall`.

---

## 3. Beginner-Friendly Explanation

Think of a Linux server like a busy restaurant kitchen:
- **The CPU:** The Head Chef. He can only chop one carrot at a time, but he switches between chopping, stirring, and plating so fast it looks like he's doing it all at once.
- **The RAM:** The kitchen counter space.
- **The Processes:** The different recipes currently being cooked.
- **The PID (Process ID):** The order ticket number assigned to each recipe so the Chef can keep track of them.
- **Process Management:** Sometimes, an order ticket gets stuck, and a pot boils over, ruining the counter space (RAM) and distracting the Chef (CPU). The Restaurant Manager (System Administrator) must identify which ticket is causing the problem and cancel the order (`kill`) so the kitchen can return to normal.
...
### 4.5 Terminating Processes: Signals
...
### The `/proc` Filesystem
How do `ps` and `top` know what processes are running? They read it from the `/proc` directory.
`/proc` is not a real directory on the hard drive. It is a **pseudo-filesystem** created dynamically by the kernel in RAM. For every running process, there is a directory named after its PID (e.g., `/proc/1234/`). Inside that directory are files detailing the process's memory maps, open file descriptors, and command-line arguments. `ps` simply formats the text found in `/proc` into a human-readable table.
...
```mermaid
graph TD
    subgraph Process_Tree ["Process Tree"]
        Kernel["Linux Kernel"]
...
│  │ │ │ │      └── The search term
│  │ │ │ └───────── Search command
│  │ │ └─────────── Pipe (sends process list to grep)
│  │ └───────────── Full format (shows PPID and arguments)
...
| `ps` | `a` | (BSD) All processes with a terminal |
| `ps` | `u` | (BSD) User-oriented format (shows %CPU/%MEM) |
| `ps` | `x` | (BSD) Include processes without a terminal (daemons) |
| `kill` | `-15` | Send SIGTERM (Terminate gracefully - Default) |
| `kill` | `-9` | Send SIGKILL (Destroy immediately) |
| `pkill` | `-u sachin` | Kill all processes owned by user `sachin` |

---

## 10. Sample Output Analysis

**Scenario:** We suspect a script is using too much CPU. We run `ps aux | grep script.py`.

**Output:**
```text
USER       PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND
sachin    4589 99.5  2.1 215432 45892 pts/0    R+   10:00   5:32 python3 script.py
```

**Analysis:**
- **USER:** `sachin` owns the process.
- **PID:** `4589` is the ID we need if we want to kill it.
- **%CPU:** `99.5%`. This script has locked up an entire CPU core in an infinite loop.
- **STAT:** `R+`. (R) Running on the CPU, (+) in the foreground group.
- **TIME:** `5:32`. It has consumed 5 minutes and 32 seconds of actual CPU processing time since it started.
- **Action:** We must terminate it by running `kill 4589`.
...
```mermaid
graph LR
    subgraph The_Kill_Signal_Workflow ["The Kill Signal Workflow"]
        Admin["Admin runs:<br/>kill -15 1234"]
        Kernel["Linux Kernel"]
        Process["PID 1234 (Database)"]
    end

    Admin -->|System Call| Kernel
    Kernel -->|Delivers SIGTERM| Process
    Process -->|Intercepts Signal| DBLogic["Database Shutdown Routine"]
    DBLogic -->|1. Flushes RAM to disk<br/>2. Closes open files<br/>3. Exits cleanly| Exit["Process Ends"]
    
    style DBLogic fill:#e1f5fe,stroke:#01579b
```
*(If `kill -9` was used, the Kernel destroys the process immediately, bypassing the Database Shutdown Routine, which can cause severe data corruption).*
...
sequenceDiagram
    participant User
    participant Shell
    participant PsCmd as ps
    participant ProcFS as /proc

    User->>Shell: ps -ef
    Shell->>PsCmd: Execute
    PsCmd->>ProcFS: Read /proc/1/cmdline
    PsCmd->>ProcFS: Read /proc/2/cmdline...
    ProcFS-->>PsCmd: Returns raw process data
    PsCmd->>PsCmd: Formats data into neat columns
    PsCmd-->>User: Displays process table
```
