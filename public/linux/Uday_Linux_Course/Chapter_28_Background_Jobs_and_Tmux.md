...
- Suspend foreground tasks (`Ctrl+Z`) and resume them in the background (`bg`).
- Bring background tasks back to the foreground (`fg`).
- Use `nohup` to protect background jobs from terminal disconnections.
- Understand the core concepts of Terminal Multiplexers.
- Create, detach, and re-attach to persistent sessions using `tmux`.

---

## 3. Beginner-Friendly Explanation

Think of your SSH terminal like a single stove burner:
- **Foreground:** You place a pot of soup on the burner and stand there stirring it. You cannot cook anything else until the soup is done.
- **Background (`&`):** You move the soup to a back burner (`Ctrl+Z` then `bg`) and let it simmer. Now the front burner is free to fry some eggs.
- **The Disconnection Problem:** If you walk out of the house (lose SSH connection), the stove automatically turns off for safety, ruining both the soup and the eggs.
- **Terminal Multiplexer (`tmux`):** You hire a robot chef. You tell the robot, "Stir this soup." You can walk out of the house, go to sleep, and come back the next day. The robot will keep cooking regardless of whether you are watching it or not.

---

## 4. Core Theory

### 4.1 Shell Job Control
...
### 4.4 Terminal Multiplexers (`tmux` / `screen`)
...
Linux traces its roots to physical typewriter terminals connected to mainframes. Every SSH connection is assigned a virtual terminal (e.g., `/dev/pts/0`). A process writes its output to this file descriptor.
When a terminal multiplexer like `tmux` starts, it creates its own internal PTYs. The processes write output to the `tmux` PTY. `tmux` then forwards that visual output to your actual SSH PTY. If your SSH PTY drops, `tmux` simply stops forwarding the visuals, but the internal PTY and the processes attached to it remain perfectly intact.
...
```mermaid
graph TD
    subgraph The_Danger_of_Standard_SSH ["The Danger of Standard SSH"]
        Laptop1["Admin Laptop"] -.->|Drops WiFi| SSH1["/dev/pts/0 (Terminates)"]
        SSH1 -.->|SIGHUP| DB_Import["Database Import (Killed!)"]
    end

    subgraph The_Safety_of_Tmux ["The Safety of Tmux"]
...
```bash
nohup rsync -avz /data/ /backup/ > /var/log/sync.log 2>&1 &
│     │                           │                     │  │
│     │                           │                     │  └── Run in background
│     │                           │                     └───── Redirect stderr to stdout
│     │                           └─────────────────────────── Redirect stdout to log file
│     └─────────────────────────────────────────────────────── The long-running command
└───────────────────────────────────────────────────────────── Ignore SIGHUP (disconnections)
```
*(Without the redirects, nohup defaults to creating `nohup.out` in the current directory).*

---

## 9. Parameter Explanation

| Command | Shortcut / Flag | Description |
|:---|:---|:---|
| `tmux` | `new -s name` | Create a named session (highly recommended) |
| `tmux` | `ls` | List all sessions |
| `tmux` | `a -t name` | Attach to the named session |
| `tmux` | `kill-session -t name` | Destroy the session and all processes inside it |
...
**Scenario:** We have multiple jobs running in our bash shell. We type `jobs`.

**Output:**
```text
[1]   Stopped                 vim /etc/httpd/conf/httpd.conf
[2]-  Running                 sleep 3600 &
[3]+  Running                 tail -f /var/log/messages &
```

**Analysis:**
- **Job 1:** A text editor. The user pressed `Ctrl+Z` to suspend it, probably to check something on the command line. (To return to it, type `fg 1`).
- **Job 2:** A long-running command executing in the background.
- **Job 3:** Another background command. The `+` sign means this is the "current" job. If the user types `fg` (without a number), Job 3 is brought to the foreground.
...
```mermaid
graph LR
    subgraph Tmux_Interface_Ctrl_b_prefix ["Tmux Interface (Ctrl+b prefix)"]
        Key["User presses Ctrl+b"]
        Key --> D["Press 'd': Detach session"]
        Key --> C["Press 'c': Create new window"]
        Key --> N["Press 'n': Next window"]
        Key --> Split["Press '%': Split pane"]
    end
```
...
sequenceDiagram
    ...
    Note over Admin,Kernel: The Safe System Upgrade
    Admin->>SSH: Connect to Production Server
    Admin->>Tmux: tmux new -s OS_Upgrade
    Tmux-->>Admin: Presents Virtual Terminal
    Admin->>Tmux: sudo dnf update -y
    Tmux->>Kernel: Starts upgrade process (takes 30 mins)
    Admin->>Admin: Laptop battery dies!
    SSH->>Tmux: Sends SIGHUP (SSH died)
    Note over Tmux,Kernel: Tmux ignores SIGHUP.<br/>Upgrade continues running safely.
    Admin->>Admin: Plugs in laptop, boots up.
    Admin->>SSH: Reconnect to Production Server
    Admin->>Tmux: tmux attach -t OS_Upgrade
    Tmux-->>Admin: Presents Virtual Terminal
    Note right of Admin: Sees "Complete!" on screen.<br/>Disaster averted.
```
