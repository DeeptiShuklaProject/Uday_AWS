...
```mermaid
graph TD
    subgraph High Availability Load Balancing
        User["Customer (Internet)"]
        VIP["Virtual IP (10.0.1.100)"]
        
        subgraph Active Load Balancer
            KA1["Keepalived (Master - Pri 100)"]
            HA1["HAProxy"]
        end
        
        subgraph Passive Load Balancer
...
```mermaid
graph LR
    subgraph The Tracking Script Failure
        User["User"]
        VIP["VIP (10.0.1.100)"]
        
        subgraph Server A (Master)
            HA1["HAProxy (CRASHED)"]
            KA1["Keepalived (Pri 100)"]
            Track["Track Script: 'killall -0 haproxy'"]
        end
        
        subgraph Server B (Backup)
            HA2["HAProxy (Running)"]
